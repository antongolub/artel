...
| V3 | **Pipeline registry + linear runs (V3.1) + parallel (V3.2.a)** | `[done]` | **V3.1:** `engine/util/pipelines.mjs` parser/validator/resolveNext; CLI `register` / `list` / `show` / `run`. JSON files at `.artel/pipelines/<id>.json` with `dispatch` + `terminal` node types and `on_disposition`-keyed edges. Walker dispatches each `dispatch` node via `dispatchLifecycle`, picks next via exact-match → wildcard fallback, emits `pipeline.registered` / `pipeline_run.started` / `.ended` (all workload). taskAttrs auto-inject `pipeline_run_id` / `pipeline_id` / `pipeline_node_id`. **V3.2.a:** `parallel` node type with `all-complete` join policy. Validator enforces non-empty branches, all branches resolvable + dispatch-typed (V3.2.a restriction; nested parallel/condition deferred), no self-references, no duplicates. Walker fans out logically (sequentially in V3.2.a — true concurrency over a shared git working tree needs worktrees, deferred to V3.3); `aggregateDisposition` does worst-of-children join (`error > timeout > parked > else success`). Each branch's taskAttrs carry `pipeline_parallel_of: <parent-node-id>` for filtering. Reachability check follows parallel branches so parallel-only flows pass validation. Show command renders parallel rows. 396 tests green (51 new across V3.1 + V3.2.a — 32 unit on validator/resolver/aggregate + 19 e2e on register/list/show/run/parallel). V3.2.b+ open: `any-complete` / `k-of-n` joins, `condition` / `pause` / `signal` / `handler` / `subpipeline`, true concurrency via git worktrees, prompt template substitution. |
