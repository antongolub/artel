...
| V3 | **Pipelines (V3.1 linear + V3.2.a parallel + V3.3.a worktrees)** | `[done]` | **V3.1:** `engine/util/pipelines.mjs` parser/validator/resolveNext; CLI `register` / `list` / `show` / `run`. JSON files at `.artel/pipelines/<id>.json` with `dispatch` + `terminal` node types and `on_disposition`-keyed edges. Walker dispatches each node via `dispatchLifecycle`, picks next via exact-match → wildcard fallback. **V3.2.a:** `parallel` node type with `all-complete` join. Validator: non-empty branches, all dispatch-typed (no nesting in V3.2.a), no self-refs, no dups. `aggregateDisposition` worst-of-children join. **V3.3.a:** git-worktree isolation lands. `engine/util/worktree.mjs` (`createWorktreeForBranch` / `removeWorktree` / `listWorktrees` / `worktreeDir`). `dispatchLifecycle` accepts `useWorktree` — creates `.artel/.worktrees/<branch>/` via `git branch -f` + `git worktree add`, runs the child with that cwd, removes on success / keeps on failure (operator can `cd` in for forensics). Dirty-tree guard skipped for worktree mode (main checkout never touched). Operator's main HEAD stays on master throughout. `git rev-parse` / `git diff --shortstat` for V10 deltas now run against the worktree, so deltas reflect the dispatch's actual diff. Pipeline `parallel` branches now run via `Promise.all` — true wall-clock concurrency, ~max(t_i) instead of sum(t_i). `artel spawn` gets `--worktree` / `--keep-worktree` flags. Test fixture's `.gitignore` now ignores `.artel/.worktrees/`. 413 tests green (68 new across V3.1 + V3.2.a + V3.3.a — 42 unit + 26 e2e). V3.3.b+ open: `any-complete` / `k-of-n` joins (need cancellation now possible with worktrees), `condition` / `pause` / `signal` / `handler` / `subpipeline`, prompt template substitution, sweep prune for stale worktrees. |
