...
//
//   node $ARTEL_HOME/engine/init.mjs                     # bootstrap or print
//   node $ARTEL_HOME/engine/init.mjs --name my-laptop    # with explicit name
//   ARTEL_PROJECT_DIR=/path node $ARTEL_HOME/engine/init.mjs
...
for (let i = 0; i < argv.length; i++) {
  if (argv[i] === '--name' && argv[i + 1]) name = argv[++i]
    ...
    console.error('Usage: node $ARTEL_HOME/engine/init.mjs [--name <cluster-name>]')
    console.error('       Idempotent — re-running prints existing identity without changes.')
    process.exit(argv[i] === '-h' || argv[i] === '--help' ? 0 : 2)
  }
}

const cluster = ensureClusterIdentity(PROJECT_ARTEL, { name })
