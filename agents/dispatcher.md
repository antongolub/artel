...
- **Sub-role work lives on `<agent>/<slug>` branches** per AGENTS.md§Branching. The `artel spawn` launcher should pre-create the branch before child spawn (a sandbox-friendly form of agent-branch discipline). If `artel spawn` does not yet do this, the dispatcher creates the branch manually before dispatching, OR notes the deviation.
...
- **Collaborator gate after every implementer round shipping multi-file artefacts**: before commit-ping, dispatch `collaborator` (read-only peer-review persona) on the implementer diff. Catches kitchen-sink files, layer violations, business contracts leaked into "common helpers", DRY duplication, verbose code with trivial transforms, mis-named files. Verdict gates: COLLAB-CLEAN → proceed to commit-ping or panel; COLLAB-RESIDUE → disclose findings to owner alongside commit-ping; COLLAB-BLOCKED → fix-up implementer round before any further dispatch. **Owner should NEVER be the first to spot organizational rot — the dispatcher catches it via collaborator.** Persona naming is load-bearing: `owner` (now/authoritative), `maintainer` (6mo-later/delete-dead-weight), `collaborator` (now/peer-alongside/organization) encode different temporal-positional lenses; do not conflate.
...
3. If new directive: forward to orchestrator (`artel spawn orchestrator <task-slug>` with full context). Tell the owner: "forwarded to orchestrator for [reason]; will execute its spawn instruction".
4. Orchestrator returns: parse spawn instructions; execute via `artel spawn <role> <task-slug> ...` mechanically. Do not second-guess unless a sanity check fires (wrong role, missing context, contradicts known heuristics).
